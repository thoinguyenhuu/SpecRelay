...
This guide installs the `v0.1.0-beta.3` CLI from a GitHub Release tarball and
...
```sh
npm install --global https://github.com/thoinguyenhuu/SpecRelay/releases/download/v0.1.0-beta.3/specrelay-cli-0.1.0-beta.3.tgz
specrelay --version
specrelay doctor --json
```

The version must be `0.1.0-beta.3`. If `specrelay` is not found, reopen the
terminal and ensure npm's global bin directory is on `PATH`.

## 2. Add the plugin marketplace

```sh
codex plugin marketplace add thoinguyenhuu/SpecRelay --ref v0.1.0-beta.3 --sparse .agents/plugins
...
- **Plugin missing:** run `codex plugin marketplace list`, confirm the
  `v0.1.0-beta.3` marketplace source, restart the desktop app, and install
